    ...
    pub fn new(json_settings_path: &str) -> Self {
        ...
        let mut base: Value = {
            ...
            serde_json::from_str(&contents).unwrap_or_else(|e| {
                panic!("Invalid JSON in {}: {}", base_path.display(), e)
            })
        };
        ...
        if let Some(ref drop_in_dir) = drop_in_dir {
            if drop_in_dir.is_dir() {
                let mut entries: Vec<_> = fs::read_dir(drop_in_dir)
                    .unwrap_or_else(|e| panic!("Cannot read drop-in dir {}: {}", drop_in_dir.display(), e))
                    .filter_map(|e| e.ok())
                ...
                for entry in entries {
                    ...
                    let patch_ops: Patch = serde_json::from_str(&contents)
                        .unwrap_or_else(|e| panic!("Invalid JSON Patch in drop-in {}: {}", p.display(), e));
                    patch(&mut base, &patch_ops)
                        .unwrap_or_else(|e| panic!("Failed to apply JSON Patch from {}: {}", p.display(), e));
                }
            }
        }

        let settings: Self = serde_json::from_value(base).unwrap_or_else(|e| {
            panic!("Merged settings are invalid: {}", e)
        });
